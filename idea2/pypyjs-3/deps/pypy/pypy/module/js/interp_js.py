...
from rpython.rlib import rgc  #AYGOR because of must be light finalizer 
...
class W_Value(W_Root):
    ...
    def __init__(self, handle):
        self.handle = handle
#AYGOR
#Class <class 'pypy.module.js.interp_js.W_Value'> is in a class hierarchy with _must_be_light_finalizer_ = True: it cannot have a finalizer without @rgc.must_be_light_finalizer
#   v20 = getattr(self_9, ('w_dict'))
#In <FunctionGraph of (pypy.interpreter.mixedmodule:121)MixedModule.getdict at 0xf09b7f0>:
#Happened at file /home/imcoelho/pypyjs/deps/pypy/pypy/interpreter/mixedmodule.py line 128
#==>         return self.w_dict
#Known variable annotations:
# self_9 = SomeInstance(can_be_None=False, classdef=pypy.interpreter.mixedmodule.MixedModule)

    @rgc.must_be_light_finalizer  #AYGOR
    def __del__(self):
...
def _convert(space, w_value):
    ...
    if space.isinstance_w(w_value, space.w_str):   #w_str or str_w ??? AYGOR
        value = space.str_w(w_value)
