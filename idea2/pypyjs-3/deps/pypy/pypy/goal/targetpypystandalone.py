...
class PyPyTarget(object):
    ...
    def target(self, driver, args):
        ...
        print("AYGOR 1 config.objspace.usemodules.thread=", config.objspace.usemodules.thread) #AYGOR
        print("AYGOR 1 parser.args=", args) #AYGOR
        assert(driver.config.objspace.usemodules.thread == False) # AYGOR
        parser = self.opt_parser(config)

        parser.parse_args(args)
        print("AYGOR 2 config.objspace.usemodules.thread=", config.objspace.usemodules.thread) #AYGOR
        ...
        print("AYGOR 3 config.objspace.usemodules.thread=", config.objspace.usemodules.thread) #AYGOR

        if config.objspace.translationmodules:
            from pypy.config.pypyoption import enable_translationmodules
            enable_translationmodules(config)

        print("AYGOR 4 config.objspace.usemodules.thread=", config.objspace.usemodules.thread) #AYGOR
        ...
        print("AYGOR if config.translation.thread=", config.translation.thread) #AYGOR
        print("AYGOR if config.objspace.usemodules.thread=", config.objspace.usemodules.thread) #AYGOR
        if config.translation.thread:
            ...
            try:
                ...
                # AYGOR
                print("will set thread option in targetpypy:", config.objspace.usemodules.thread)
